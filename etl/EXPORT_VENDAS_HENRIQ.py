...
DIRETORIO_SAIDA = Path(r"C:\Users\emanu\Documents\Desenvolvimento\Business Flow\etl_testes\exports") # Define o diretório onde os arquivos serão salvos
PREFIXO_ARQUIVO = "DADOS_TRANSFORMADOS_VENDAS"
...
def processar_dados() -> None:
    ...
    # Mantemos datetime inicialmente para facilitar a extração do ano depois
    df_vendas['data_venda_dt'] = pd.to_datetime(df_vendas_base['DATA_VENDA'])
    df_vendas['data_venda'] = df_vendas['data_venda_dt'].dt.date
    df_vendas['ano_venda'] = df_vendas['data_venda_dt'].dt.year # Coluna auxiliar para o filtro
    ...
    # =================================================================
    # Identifica todos os anos únicos na base
    ...
    for ano in anos_unicos:
        ...
        # 1. Filtra as vendas do ano atual
        vendas_ano = df_vendas[df_vendas['ano_venda'] == ano].copy()
        
        # Captura os IDs das vendas desse ano para filtrar as tabelas filhas
        ids_venda_ano = vendas_ano['id_legado'].unique()
        
        # 2. Filtra itens e pagamentos vinculados apenas a essas vendas
        ...
        # Remove as colunas auxiliares antes de exportar
        vendas_ano = vendas_ano.drop(columns=['data_venda_dt', 'ano_venda'])

        # Monta o nome do arquivo dinâmico
    ...
    print("\nTodo o pipeline de ETL foi concluído com sucesso!")
