<template>
  <section>
    <BaseTable
      ...
      subtitle="Consulta detalhada dos itens consolidados"
      :columns="columns"
      :rows="rows"
      row-key="id_item_compra"
      :count="count"
      :next="next"
      :previous="previous"
      :loading="loading"
        ...
        <div class="flex flex-wrap items-center gap-2">
          <div class="flex items-center gap-2 rounded-md border border-gray-200 bg-white px-2 py-1.5">
            ...
            <input
              v-model="search"
              type="text"
              placeholder="Buscar item, produto ou fornecedor"
...
const endpoint = `${API_BASE_URL}/api/compras/itens`;
...
const columns = [
  ...
  { key: "valor_total_item", label: "Total" },
  { key: "nfe_status", label: "Status NFe" },
];

const exportColumns = [
  { key: "id_item_compra", label: "ID Item Compra" },
  { key: "compra", label: "Compra" },
  { key: "produto", label: "Produto" },
  { key: "unidade_medida", label: "Unidade" },
  { key: "quantidade", label: "Quantidade" },
  { key: "valor_custo", label: "Valor Custo" },
  { key: "valor_total_item", label: "Valor Total Item" },
  { key: "descricao_origem", label: "Descricao Origem" },
  { key: "descricao_compra_origem", label: "Descricao Compra Origem" },
];
...
function formatCompraRef(row) {
  return `COMPRA #${row?.id_legado_compra ?? ""}`;
}
...
async function load(url = endpoint) {
  ...
  try {
    ...
    console.error(err);
    error.value = "Falha ao carregar itens de compra.";
  } finally {
    loading.value = false;
  }
}
...
function openDetails(row) {
  activeCompraId.value = row.compra;
...
async function exportData({ tipo, colunas, query_sql, formato, salvar_nome }) {
  ...
  try {
    ...
    const filename = getFilenameFromDisposition(response.headers.get("Content-Disposition"), `exportacao_itens_compra.${formato}`);
