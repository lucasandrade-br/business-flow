...
const barChart = computed(() => {
  ...
  const vals = dados.flatMap(d => [d.receita_atual, d.receita_anterior]).filter(v => v != null && v > 0)
  ...
  const bars = dados.map((d, i) => {
    ...
    const hAnt = bh(d.receita_anterior)
    const hAtu = bh(d.receita_atual)
    return {
...
const ticker = computed(() => {
  ...
  return dados.map(d => {
    ...
    const ra = isAtual ? Number(kpis.value.mtd_receita_atual || 0)                : (d.receita_atual    ?? null)
    const rb = isAtual ? Number(kpis.value.mtd_receita_anterior_equivalente || 0) : (d.receita_anterior ?? null)
    const pct = ra != null && rb != null ? calcPct(ra, rb) : null
    return {
      label: d.label, mes: d.mes, pct, isAtual,
      positivo: pct != null && Number(pct) >= 0,
      show: ra != null,
      valorAtual:    ra != null ? fmtMoneyFull(ra) : null,
      valorAnterior: rb != null ? fmtMoneyFull(rb) : null,
...
<template>
  <div class="flex flex-col gap-6">
    ...
    <div>
      ...
      <p class="text-sm text-gray-400 mt-0.5">Receita e volume de vendas acumulado no ano.</p>
    </div>
    ...
    <div v-else-if="semDados" class="rounded-xl border border-dashed border-gray-200 bg-white px-6 py-8 text-center">
      ...
      <p class="text-xs text-gray-400 mt-1">Execute o sistema ou consolide vendas para gerar o painel.</p>
    </div>
    ...
    <div v-else-if="kpis" class="rounded-xl border border-gray-200 bg-white shadow-sm">
      ...
      <div class="flex items-center justify-between rounded-t-xl bg-gradient-to-r from-gray-50 to-white px-4 py-2.5 border-b border-gray-100">
        <div class="flex items-center gap-2">
          ...
          <span class="text-xs font-bold uppercase tracking-wider text-[#373435]">Análise de Vendas</span>
        </div>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-[260px_1fr] divide-y lg:divide-y-0 lg:divide-x divide-gray-100">
        ...
        <div class="p-4 flex flex-col gap-3">
          ...
          <div class="flex items-center justify-between">
            <p class="text-[10px] font-semibold uppercase tracking-wider text-gray-400">Receita Mensal</p>
            <div class="flex items-center gap-4">
              ...
              <div class="flex items-center gap-1.5">
                <div class="h-2.5 w-2.5 rounded-sm" style="background:#2f6f4f" />
            ...
            >
              ...
              <g v-for="b in barChart.bars" :key="b.label">
                <rect
                  ...
                  fill="#2f6f4f" rx="1.5" ry="1.5"
                />
                <rect
                  v-if="b.ant.h > 0"
                  :x="b.ant.x" :y="b.ant.y"
                  :width="b.ant.w" :height="b.ant.h"
                  fill="#2f6f4f" fill-opacity="0.3"
                  rx="1.5" ry="1.5"
                />
                <text
          ...
          <div class="flex flex-wrap gap-1.5">
            ...
            >
              ...
              <!-- Tooltip com receita de ambos os anos -->
              <div class="pointer-events-none absolute bottom-full left-1/2 -translate-x-1/2 mb-2 hidden group-hover:block z-20">
                <div class="relative bg-gray-900 text-white rounded-lg px-3 py-2 text-[11px] shadow-xl whitespace-nowrap min-w-[160px]">
                  ...
                  <div class="flex items-center justify-between gap-6">
                    <span class="text-emerald-400 font-semibold">{{ t.anoAtual }}</span>
