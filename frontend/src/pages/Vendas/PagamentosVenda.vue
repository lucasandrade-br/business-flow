<template>
  <section>
    <BaseTable
      ...
      subtitle="Consulta de formas de pagamento por venda"
      :columns="columns"
      :rows="rows"
      row-key="id_pagamento_venda"
      :count="count"
      :next="next"
      :previous="previous"
      :loading="loading"
    ...
    >
        ...
        <div class="flex flex-wrap items-center gap-2">
          <div class="flex items-center gap-2 rounded-md border border-gray-200 bg-white px-2 py-1.5">
            ...
            <input
              v-model="search"
              type="text"
              placeholder="Buscar pagamento, cliente ou tipo"
      ...
      <template #cell-estorno="{ row }">
        <span
          class="inline-flex rounded-full px-2 py-0.5 text-[10px] font-semibold"
          :class="row.estorno ? 'bg-amber-100 text-amber-700' : 'bg-green-100 text-green-700'"
        >
          {{ row.estorno ? 'Sim' : 'Nao' }}
        </span>
      </template>
...
const exportColumns = [
  { key: "id_pagamento_venda", label: "ID Pagamento" },
...
async function load(url = endpoint) {
  ...
  try {
    ...
    console.error(err);
    error.value = "Falha ao carregar pagamentos de venda.";
  } finally {
    loading.value = false;
  }
}
...
async function exportData({ tipo, colunas, query_sql, formato, salvar_nome }) {
  ...
  try {
    ...
    const filename = getFilenameFromDisposition(response.headers.get("Content-Disposition"), `exportacao_pagamentos_venda.${formato}`);
